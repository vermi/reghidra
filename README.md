...
- **Bundled type archives** — ~20 k Win32 prototypes (windows-sys) for x86/x64/ARM64 plus ~450 POSIX prototypes (libc) shipped as postcard-serialized `.rtarch` blobs, auto-selected by binary format + architecture. Drives arity capping on stack-arg collapse and surfaces parameter types at call sites as casts (`TerminateProcess((HANDLE)hProc, (DWORD)exit_code)`). Cross-block pending propagation recovers arg lists that straddle basic-block boundaries (the canonical Win32 termination idiom where `push; call` is split by the lifter).
