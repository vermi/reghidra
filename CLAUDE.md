...
6. **Return-type propagation (still pending).** For call expressions in the decompile output where the target has a known prototype, propagate the return type back onto the LHS var of the assignment. `eax = CreateFileA(...)` becomes `HANDLE hFile = CreateFileA(...)` at the point of first assignment. Not yet implemented.
