...
### Phase 5c — Type system follow-ups [DONE]

Closed out items 1a–1c of the remaining-work list below: slot subsumption, user-retype flow-through to call-site casts, and a type picker dropdown for the "Set Type..." popup.

- **Slot subsumption.** `FrameLayout` gained `subsumed_by: BTreeMap<i64, i64>` (child offset → parent offset) and a `retype_slot(offset, new_ctype, new_size) -> Vec<i64>` API that walks siblings in `[offset+old_size, offset+new_size)` and marks them subsumed when the new type is wider than the slot's existing width. Children stay in `slots` for undoability — only `prepend_var_decls` and `apply_user_variable_types` filter them out at emit time. Width resolution goes through `type_archive::ctype_size_bytes` which follows alias chains (windows-x64 + built-in `DWORD`/`HANDLE`/`size_t` table) so retyping to `Named("HANDLE")` correctly widens to 8 bytes. Unknown widths fall back to type-only retype with no subsumption — opaque struct names don't accidentally eat their neighbors.
- **Cast strip post-pass.** `expr_builder::strip_compatible_call_casts` runs as a new Step 6.5 in `decompile()` / `decompile_annotated()`, after `apply_user_variable_types`. It walks every `Expr::Call` in the body, builds a `local_types: HashMap<String, CType>` from the body's surviving `VarDecl`s, and unwraps any `Cast(target, Var(name))` whose source type is assignment-compatible with `target`. The compatibility predicate (`type_archive::is_assignment_compatible`) handles named-alias resolution through archive `types`, same-width / same-signedness integer matching, `Unknown(n)` wildcards at matching width, and pointer-to-X recursion. Conservative on `IntLit` sources: literal casts like `(DWORD)0xc0000409` keep their cast for semantic intent. Known limitation: the lifter inserts an intermediate register copy between a slot read and a `push`, so the call site syntactically references `result`/`var_N` rather than `arg_8`. The strip pass operates on syntactic Var-name lookups and can't follow that one-step copy chain — `tests/cast_strip.rs::matching_retype_through_lifter_intermediate_keeps_cast_pinned_limitation` pins the current behavior so we know when it changes. Closing this gap needs a copy-propagation pass that's a follow-up of its own.
- **Type picker dropdown.** `Project::known_type_names()` returns a sorted union of: C primitives, common Win32 / POSIX width aliases (`HANDLE`, `DWORD`, `BOOL`, `LPCSTR`, `wchar_t`, ...), and every `TypeDef` key from each loaded `TypeArchive.types`. The "Set Type..." popup renders an `egui::ComboBox` above the existing `TextEdit` populated from this list; selection writes back to `self.text` so the user can still hand-edit afterwards. A live parse-feedback line under the input shows whether the entered text resolves to a recognized variant or falls through to `CType::Named(...)`. The free-form text edit stays as the canonical commit surface so users typing custom struct names not in any archive still get a fallback path. `parse_user_ctype` and `CType` are reachable from the GUI via a new `reghidra_core::ast` re-export module — the GUI doesn't need a direct dep on `reghidra-decompile`.
- **Pipeline ordering.**
  ```
  ...
  Step 6: expr_builder::apply_user_variable_types  (now: &mut FrameLayout — slot subsumption hook)
  Step 6.5: expr_builder::strip_compatible_call_casts (new)
  Step 7: emit::emit_function
  ```
  Both `decompile` and `decompile_annotated` get the same change. `frame_layout` is now plumbed `&mut` through Step 6 because `retype_slot` mutates the `slots`/`subsumed_by` maps in place; the post-pass layout flows back into `DecompileOutput.frame_layout` so the GUI sees the merged state.
- **Tests added.** 4 unit (`stackframe.rs::retype_slot_*`, `prepend_var_decls_skips_subsumed_children`), 5 unit (`type_archive::tests::compat_*`, `ctype_size_bytes_resolves_known_names`), 2 unit (`expr_builder::tests::strip_pass_removes_matching_cast`, `strip_pass_keeps_mismatched_cast`), and two new e2e files: `tests/slot_subsumption.rs` (3 tests, full pipeline) and `tests/cast_strip.rs` (3 tests, including the lifter-intermediate limitation pin).

### Phase 5c — GUI perf hardening [DONE]

User reported the GUI burning 80% CPU + 2 GB RAM on `pe-mingw32-strip.exe`. Bisecting found four independent culprits, all unrelated to the type-system PR — they pre-dated it on `main`.

- **Side panel virtualization.** All six `views/side_panel.rs` panels (Functions, Symbols, Imports, Exports, Sections, Strings) were using `ScrollArea::vertical().show()` which lays out every row every frame. On a 1393-function PE that's 1393 `selectable_label` widgets per panel per frame plus their `address_context_menu` setup — egui then caches per-widget state for thousands of off-screen rows and the resident memory grows linearly. Switched all six to `ScrollArea::vertical().show_rows(ui, ROW_HEIGHT, len, |ui, range| { ... })` which only renders the visible window. `SIDE_PANEL_ROW_HEIGHT` is the shared constant. Panels with inline `query` filters (Sections used a `continue` inside the loop) were restructured to pre-filter into a `Vec<_>` first so `show_rows` can index by row.
- **Disasm display-list cache.** `views/disasm.rs` was rebuilding the entire `display_lines: Vec<DisplayLine>` from all 226 k instructions every frame, calling `function_at` + `ref_count_to` + `bookmarks.contains` + `demangle::display_name_short` for each — about 13 M operations/sec at 60 fps. `build_display_lines` is now a free function and the result is cached on `ReghidraApp::disasm_lines_cache: Option<(u64, Box<dyn Any + Send + Sync>)>` keyed on a new `disasm_display_generation` counter. The counter bumps on file open, function rename, bookmark toggle, and undo/redo (the introspection helper only checks renames, so we conservatively bump on every undo). Comments don't structurally affect the display list (they render inline within the instruction row) and don't bump it. The cache payload is `Box<dyn Any>` so `app.rs` doesn't need to know the `DisplayLine` enum's shape — `views/disasm.rs::render` downcast-refs at use site.
- **Decompile reverse-map cache.** `views/decompile.rs` rebuilt a `func_name_to_addr: HashMap<String, u64>` every frame by demangling all 1393 function names. Cached on `ReghidraApp::func_name_to_addr_cache` keyed on `rename_generation` (function renames are the only mutation that affects it). Re-borrow dance after the cache write unwinds the `&mut self` borrow before the rest of the render reads `&project`.
- **Conditional repaint scheduling.** The biggest win: `app.rs::update` was calling `ctx.request_repaint_after(Duration::from_millis(33))` *unconditionally* on every frame, pinning egui to a 30 fps continuous redraw loop forever — even at idle, even with no input, burning ~40% CPU and accumulating egui's per-frame allocator scratch state in resident memory. Replaced with conditional scheduling: only request a follow-up repaint when there's actual pending work (`hovered_address.is_some()` after the double-buffer promote, or a `status_message` deadline that hasn't fired yet). Mouse moves, clicks, key presses, and scroll wheels are handled by egui's reactive mode automatically — they trigger repaints via the input path, no help needed from us. Idle CPU dropped from ~40% to ~2%.
- **Where this lives in the branch graph.** Originally landed alongside the type-system follow-ups on `phase5c-type-system-followups` while bisecting the user report. Split out into its own PR before merge so the type-system work and the perf hardening can be reviewed independently.

**Remaining Phase 5c work — priorities.** Target workload is PE-heavy with some ELF/Mach-O; ARM is out of scope.

1. **Mach-O symbol coverage** (when a real Mach-O target appears):
...
3. **Intelligent FLIRT pre-selection.** `bundled_sigs::collect_bundled_sigs` currently loads *every* `.sig` file in the format/arch subdir unconditionally — on a PE x86 binary that's 74 sigs spanning Borland, Watcom, MSVC, MinGW, Delphi/BCB regardless of which toolchain actually built the binary. The trie expansion blows resident memory up by ~25-30x on-disk size, so on `pe-mingw32-strip.exe` Project::open allocates ~1.3 GB just for the bundled FLIRT trees even though only ~163 functions match across all 74 dbs combined. FLIRT pattern matching makes wrong-toolchain sigs harmless at the data layer (they just don't match), but it wastes parse time, pollutes the panel, clouds the precedence story, AND now is the dominant memory consumer at idle. A smart selection should key off `BinaryInfo` signals: PE Rich Header `@comp.id` records (exact MSVC version), PE imports (`msvcr*.dll` / `vcruntime140.dll` / `ucrtbase.dll` / `libgcc_s_*.dll`), ELF `.note.ABI-tag` and `DT_NEEDED libc.so.6` vs `ld-musl-*`, Mach-O `LC_LOAD_DYLIB` of `libSystem.B.dylib`. The Loaded Data Sources panel's lazy-load opt-in (PR 7) is the natural escape hatch when the heuristic is wrong: auto-load the strong-signal sigs, leave the rest available-but-unchecked. Implementation lives in `bundled_sigs::auto_load_subdirs` (or a per-sig predicate replacing `sig_subdirs`'s blanket enumeration).
   - **Memory budget context.** Per-component breakdown on `pe-mingw32-strip.exe` (measured via a throwaway perf test): binary load 7 MB → +disasm 547 MB → +bundled FLIRT (74 dbs) 1342 MB → +analyze 141 MB → total ~2 GB. The disasm step is also surprisingly heavy (~2.4 KB per `DisassembledInstruction` after capstone's `.detail(true)` mode plus the per-insn `Vec<u8>` / `String` / `String` allocator overhead) — orthogonal optimization, not gating the FLIRT work.
4. **Hint hit source via color/badge in the views.** Right now a function name in the disasm view, decompile view, or side panel renders the same regardless of *which* data source produced it — FLIRT match vs typed import vs heuristic `sub_XXXX` are visually indistinguishable. Add a per-source color (or a small badge / icon column / hover tooltip) so the user can see at a glance "this name came from `vc32_14` FLIRT" vs "this prototype came from `windows-x64` archive". The data is already on `Function::matched_signature_db` (FLIRT) and resolvable via `type_archive::which_archive_resolves` (archives); the work is purely render-side. Should be consistent across the disasm `FuncHeaderName` row, the decompile call-site tokens, and the side panel function list. The Loaded Data Sources panel's per-source color (if we add one) becomes the legend.
