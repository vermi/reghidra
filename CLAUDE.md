...
- **Rizin `librz/arch/types/*.sdb.txt`**: ~90 hand-curated SDB text files covering Windows APIs (34 `functions-windows_*` split per-header), POSIX/libc/Linux/macOS/Android, x86/ARM/MIPS calling conventions. Trivially parseable flat `key=value` format. GPLv3 licensed — bundleable since Phase 5c PR 4g relicense. **Heads-up from the PR 4h exploration**: Rizin's SDB files do NOT contain the MSVC CRT internals (`__SEH_prolog4`, `__EH4_*`, `__lockexit`, `__mtinit`, ...) — they're absent from `functions-libc.sdb.txt` and there's no `functions-msvcrt.sdb.txt` either. The original motivation "Rizin will close the MSVC CRT internals gap" was optimistic. The walker is still worth having because Rizin's per-header Windows coverage (`functions-windows_winhttp.sdb.txt`, `functions-windows_winsock2.sdb.txt`, etc.) is orthogonal to `windows-sys` and sometimes richer. Implementation landed in PR 4h: `tools/typegen/src/walker/rizin_sdb.rs` parses the flat SDB format into `FunctionType` records, registered as `--source rizin-sdb --input <path>` where `<path>` is a single `.sdb.txt` file or a directory containing one or more (typically a Rizin checkout's `librz/arch/types/`). No archives have been generated and checked in yet — that's a maintainer-run step that waits on a decision about archive slicing strategy. **Attribution requirement when generating**: GPLv3 obligates us to preserve Rizin's copyright notices in any derived archive; the walker encodes the source in the archive's `name` field and the LICENSE file should be updated to call out the Rizin import when an archive lands under `types/`.
