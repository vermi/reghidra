...
### Phase 5e — YAML Detection Rules [DONE]

Native YAML rule engine for malicious/suspicious behavior detection, joining FLIRT and type archives as a third data source. Rules are capa-shaped: feature expressions (`import`/`api`/`string`/`name`/`section`/`mnemonic`/`mnemonic_sequence`/`xrefs_to`/`xrefs_from`/`rich_comp_id`/`imphash`/`tls_callbacks`/`overlay`/`matches`) composed with `and`/`or`/`not`/`n_or_more`/`count`. Ships 49 bundled rules across `rules/{anti_analysis,crypto,injection,network,packers,persistence,suspicious_api}/`.

**Engine.** `reghidra-detect` crate: `rule.rs` (AST), `parser.rs` (YAML → compiled Rule, regexes precompiled at load time), `features.rs` (snapshot types), `eval.rs` (two-pass evaluator — pass 1 function + plain file-scope, pass 2 file-scope rules that use `matches:`), `bundled.rs` (`include_dir!("../../rules")` + `available_bundled_rulefiles()` without parsing), `entropy.rs` (Shannon helper). `Section.entropy: f64` populated on every `LoadedBinary` at load time across ELF/PE/Mach-O paths.

**Pipeline.** `Project::evaluate_detections()` runs as a post-analysis step after FLIRT/archive application. Builds a `Features` snapshot from analyzed state — per-function mnemonics from `analysis.cfgs[entry].blocks` in BTreeMap order, APIs from resolved callee names, string refs from `XRefKind::StringRef` xrefs against `binary.strings`, xref counts. Evaluates enabled rules, stamps `source_path` on hits, computes `per_rule_file_counts`, bumps `detections_generation`. Called on `Project::open`, reanalyze, and every rule-file enable/disable.

**Known gaps.** `Features.file.imports` `lib` field is always `""` (DLL stem metadata isn't plumbed through); rules should prefer `api:` over `import:`. Byte-pattern scanning deferred to v2 — FLIRT already covers byte signatures. Addressing-mode features (`fs:[30h]` PEB walk etc.) unmodeled; affected bundled rules (`peb-beingdebugged`, `ntqueryinformationprocess-debug`) approximate via API/string combos and are intentionally low-severity.

**Data source attribution.** `LoadedRuleFile { source_path, rules, enabled, parse_errors }` on `Project::loaded_rule_files`. Bundled rules identified by `source_path = "bundled:<subdir>/<stem>.yml"`. Lazy-load + enable/disable follow the Phase 5c pattern (enumerate-all via `available_bundled_rulefiles()` without parsing; load on first enable via `Project::load_bundled_rule_file(subdir, stem)`; user files via `load_user_rule_file(path)`; toggle via `set_rule_file_enabled`). Parse errors on individual rule files surface via `parse_errors` and don't block other files. `include_dir!` caches at compile time — if a newly-dropped rule file isn't picked up, `touch crates/reghidra-detect/src/bundled.rs` forces recompile.

**Session persistence.** `Session` carries `loaded_bundled_rule_stems: Vec<(String, String)>`, `loaded_user_rule_paths: Vec<PathBuf>`, `rule_file_overrides: Vec<DataSourceOverride>` (reusing the existing `{kind: "rule", key, enabled}` struct). All `#[serde(default)]`. Replay order in `apply_session`: bundled → user → overrides, with a single `evaluate_detections` pass at the end.

**CLI.** `reghidra-cli detect list [--severity] [--rule REGEX] [--function ADDR-OR-NAME] [--json]`. `reghidra-cli sources rules {list,available,load,load-user-file,enable,disable,resolve}`. Mutations require `--session` (same contract as `sources flirt`/`sources archives`). `detect list` JSON shape pinned by `crates/reghidra-cli/tests/cli.rs`.

**GUI.** `SidePanel::Detections` tab grouped by severity with virtualized rows via `show_rows`. Function-row badge (severity-tinted dot) in the Functions panel. Decompile-view banner at the top of detected functions showing `⚠ N detection(s): rule-a · rule-b` with rule names painted in severity colors. Data Sources panel's Detection Rules section uses the action-queue pattern for per-rule-file enable/disable and "Load bundled/user file…" affordances. Command palette entries: "Toggle Detections Panel", "Reload Detection Rules", "Load Detection Rules…". `detections_generation` invalidates the disasm minimap lane and any caches that display detection state.

**Authoring.** `rules/README.md` is the rule-author guide (anatomy, feature reference, combinators, scope/severity guidance, testing, style, contribution checklist). Every bundled rule has two unit tests (fires + no-fire) in `crates/reghidra-detect/tests/bundled_rules.rs` using synthetic `Features` (`one_fn`, `file_feats`, `file_feats_with_sections`, `file_feats_with_pe` helpers — do NOT duplicate). `every_bundled_rulefile_parses` is the sanity canary. Regression canary: `crates/reghidra-core/tests/detect_pipeline.rs::wildfire_fixture_fires_expected_rules` pins the set of rules firing on the primary fixture — update it when intentionally changing a rule's firing behavior.

**Follow-ups.** egui_kittest harness for GUI regression tests (separate PR). Plumb `lib` metadata into `Features.imports`. Byte-pattern `pattern:` feature. Entropy visualizer view (sliding-window graph across file).
